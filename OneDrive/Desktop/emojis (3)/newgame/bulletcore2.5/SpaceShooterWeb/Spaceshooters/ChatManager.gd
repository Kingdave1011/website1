...
signal message_received(sender_name: String, message: String, timestamp: String, message_type: String)
...
# Called when player connects to game
func announce_player_join(player_name: String):
	var timestamp = Time.get_time_string_from_system()
	var join_message = player_name + " joined the game"
	_broadcast_system_message.rpc(join_message, timestamp, "join")
	player_joined.emit(player_name)

# Called when player disconnects
func announce_player_leave(player_name: String):
	var timestamp = Time.get_time_string_from_system()
	var leave_message = player_name + " left the game"
	_broadcast_system_message.rpc(leave_message, timestamp, "leave")
...
func _broadcast_message(sender_name: String, message: String, timestamp: String):
	...
	message_received.emit(sender_name, message, timestamp, "chat")

@rpc("authority", "call_local", "reliable")
func _broadcast_system_message(message: String, timestamp: String, msg_type: String):
	chat_history.append({
		"sender": "SYSTEM",
		"message": message,
		"timestamp": timestamp,
	...
	message_received.emit("SYSTEM", message, timestamp, msg_type)
